...
- applications
  ...
  - each app have a unique id attached to it
- nats-jet-stream
...
- relay-server
  ...
  - sse connections should be reusable based on the timing parameters
  ...
  - this must have a different kinds of events types
    ...
    - keep-alive connection
      - we need to figure out, keep alive should be property of the http connection
  ...
  - automatic reconnect support
    ...
    - when reconnecting browser send the last-event-id in the header or query
  ...
  - request may include the parameter
    - since=instant,start,duration(5m,1h,30s,2d, etc) and ts=<datetime timestamp>
